...
void HostSettingKanjiCodeForm::updateUi(const BaseForm::Data& data) {
  ...
  const std::map<KanjiCode, const TValue> host_codes{
      ...
      {KanjiCode::kUTF8N, {*d_->ui.radioButton_1_5_UTF8, false}},
  ...
  UI_SETCHECKED(d_->ui.checkBox_1_1_HKanaToW, tdata.kanacnv);
  UI_SETENABLED(d_->ui.checkBox_1_1_HKanaToW,
                host_codes.find(tdata.kanjicode)->second.conv);
  name_codes.at(tdata.kanjicode_name).rbtn.setChecked(true);
  UI_SETCHECKED(d_->ui.checkBox_2_1_HKanaToW, tdata.kanacnv_name);
  UI_SETENABLED(d_->ui.checkBox_2_1_HKanaToW,
...
void HostSettingKanjiCodeForm::updateData(BaseForm::Data& data) const {
  ...
  const std::map<KanjiCode, const QRadioButton&> host_codes{
      ...
      {KanjiCode::kUTF8N, *d_->ui.radioButton_1_5_UTF8},
  ...
  UI_ISCHECKED(tdata.kanacnv, d_->ui.checkBox_1_1_HKanaToW);
  ...
  UI_ISCHECKED(tdata.kanacnv_name, d_->ui.checkBox_2_1_HKanaToW);
}
