...
class DirsAndTaskMsgForm::Private {
 public:
  Private(QWidget* parent)
      : mymodellocal(nullptr), mymodelremote(nullptr), parent_(parent) {}
  ~Private() {}
...
void DirsAndTaskMsgForm::Private::resetModelLocal(const QString& path,
                                                  bool isshowndot) {
...
void DirsAndTaskMsgForm::Private::resetModelRemote(const QString& path,
                                                   bool isshowndot) {
  saveColumnsWidthRemote();
  ui.treeView_RemoteDir->setModel(nullptr);
  delete mymodelremote;
  // mymodelremote = new MyFileSystemModelRemote(parent_, path, isshowndot); //
  // TODO: MyFileSystemModelRemoteクラスができたら
...
void DirsAndTaskMsgForm::setDotFileShown(bool isshown) {
  // d_->mymodellocal->setShowDot(isshown); //
  // 動的に切り替えようとすると落ちるのでモデル自体を作り直す↓
...
void DirsAndTaskMsgForm::onClicked_toolButton_LocalUp() {
  ...
  d_->resetModelLocal(
      reinterpret_cast<MyFileSystemModelLocal*>(d_->mymodellocal)->oneUpPath(),
      d_->mymodellocal->showDot());
}
...
void DirsAndTaskMsgForm::onCurrentIndexChanged_comboBox_LocalDir(int index) {
  qDebug() << __FUNCTION__ << "called. index=" << index
           << ", text=" << d_->ui.comboBox_LocalDir->itemText(index);
}

void DirsAndTaskMsgForm::onDoubleClicked_treeView_LocalDir(
    const QModelIndex& index) {
  qDebug() << __FUNCTION__
           << "called. fullpath=" << d_->mymodellocal->fullPath(index)
           << "isdir=" << d_->mymodellocal->isDir(index);
  if (d_->mymodellocal->isDir(index)) {
    d_->resetModelLocal(d_->mymodellocal->fullPath(index),
                        d_->mymodellocal->showDot());
  }
}
...
void DirsAndTaskMsgForm::onDoubleClicked_treeView_RemoteDir(
    const QModelIndex& index) {
