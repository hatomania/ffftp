...
なるべく元のコードはいじらないという方針のもと、Win32 APIの呼び出し自体は~~削除せずに、スタブを作成し、スタブ呼び出しに置き換える。置き換え対象のWin32 APIの機能が複雑で実装が難しい場合は~~そのWin32 API呼び出している関数自体を書き換える~~ことも視野に入れる~~。

Windows専用定義（Win32 APIやWindows専用の型）はそれを使用している宣言や定義を全てdefineで切り分けて書き換えるものとする。当初は可能な限りスタブ化をしようと考えていたが、サウンドのレジストリ周りや、ゾーンID設定、タスクバープログレスなど、Windows版ではWindows専用定義を使用しなければ実現できない機能があり、特定のWindows専用定義だけスタブ化するのは困難なため。
...
通信の進捗状態はlibffftpから受け取れるようにする。

libffftpはあくまでウィンドウの処理系に依存しないライブラリであるため。
